...
分からない(0)は次のラウンド内(最低3枚後)のランダムな位置に、曖昧(40)は次の
ラウンドかそれより数ラウンド後(50%の確率でどちらか)の区間にランダムな位置に
再挿入します(`src/lib/queue.ts`の`requeuePosition`)。ラウンドサイズは下記
「マイクロラーニング」でユーザーが自由に調整でき、再登場の間隔もその値に従います。
...
모름(0)은 다음 라운드 안(최소 3장 뒤)에서 무작위 위치에, 헷갈림(40)은 다음
라운드 또는 그보다 몇 라운드 뒤(50% 확률로 둘 중 하나) 구간에서 무작위 위치에
다시 꽂습니다(`src/lib/queue.ts`의 `requeuePosition`). 라운드 크기는 아래
"마이크로 러닝"에서 사용자가 직접 조절할 수 있어, 재등장 간격도 그 값을 그대로
따라갑니다.
