...
| 少し分かる | 60 | 次の3~5ラウンド後の区間 | 曖昧より余裕を持って、もう一度 |
| 曖昧 | 40 | 次のラウンドまたは3~5ラウンド後(50%) | 少し分かるより早めに再登場 |
| 分からない | 0 | 今回のラウンド内(最低3枚後) | すぐに再登場 |

完璧だけを「今回はもう十分」として即座に完了扱いにし、進捗率に反映します。少し
分かる・曖昧・分からないは全て今回のセッションキューに再挿入され、もう一度出題
されます — その中でも少し分かるは既によく分かっている単語なので、曖昧より余裕を
持たせた(3~5ラウンド後)区間でのみ再登場します(即座に完了扱いにするのは惜しいと
いう声を反映し、「それでももう一度だけ確認してから」に変更しました。以前は完璧
だけを完了として数えていたため進捗率がなかなか上がらないという逆の声もありました
が、今は完璧だけを即座に完了とし、残り3段階は全て再登場させることで両方の要望の
バランスを取っています)。

分からない(0)は今回のラウンド内(最低3枚後)のランダムな位置に、曖昧(40)は次の
ラウンドかそれより数ラウンド後(50%の確率でどちらか)の区間に、少し分かる(60)は
常にそれより後の3~5ラウンド後の区間(曖昧の範囲と重なっても問題なし)にランダムな
位置に再挿入します(`src/lib/queue.ts`の`requeuePosition`/`requeueRangeBounds` —
採点ボタンに表示される「N~M番目」の数字もこの関数一つからそのまま取っているため、
ラベルと実際の動作は常に一致します)。ラウンドサイズは下記「マイクロラーニング」で
ユーザーが自由に調整でき、再登場の間隔もその値に従います。

三つの設計意図があります。

- 固定値ではなく区間からランダムに選びます。位置を固定するとユーザーが順番を覚えて
  しまい、単語ではなく「順番」を記憶してしまいます。
- 絶対数ではなくラウンドサイズを基準に計算します。ラウンドを大きく設定した人ほど、
  間違えた単語が戻ってくる間隔も自然に長くなります。
- 採点直後にその単語がすぐ次のカードとして再登場しないよう、最低3枚の間隔
  (`MIN_GAP`)を常に保証します。
...
### マイクロラーニング — 調整可能なラウンドとよく間違える単語

一度に数百語をまとめて解かせると、途中で疲れてしまいがちです。そこで練習を
「ラウンド」単位に分割しました。デフォルトは15語ですが、1日30語だけ軽く行う人と
200語以上まとめて行う人ではリズムが違うため、練習画面右上の「⚙設定」ポップオーバー
から5〜50語の間で自由に調整できます(端末ごとに保存、学習記録とは無関係)。
ラウンドを終えるたびに短い完了画面が表示され、画面上部の進捗バー(ジェリー
スタイル)も全体ではなくそのラウンド基準で満ちていきます。3回以上曖昧・分からないと
採点された単語は、正解画面に「よく間違える単語」バッジが付き、どの単語が特に
覚えにくいかすぐに分かります。

### 間隔反復(SRS) — 今日の復習

完璧(100点)と採点した単語は7日後、少し分かる(60点)と採点した単語は3日後に
再び出題されるよう、固定間隔を適用します(以前はSuperMemo-2のように正解するたびに
間隔が広がる方式でしたが、4段階評価と噛み合わず点数ごとの固定値に単純化しました)。
復習日がまだ来ていない単語はその日の練習スタックから完全に除外され、間隔が来た
単語は「復習」画面の「今日の復習」カードに集まり、今すぐ見直すべき単語だけを
選んで練習を始められます。
...
- ドロップダウンで組み合わせを選び**[この学習をもう一度]**を押すと、そのときの
  ...
  画面まで進む — その組み合わせに保存された進捗があれば(他の端末で保存したもので
  あっても)最初からシャッフルし直さず、キューと完了数を正確にその地点から
  引き継ぎます。

ファイルの組み合わせごとの進捗記録は(ユーザー番号、パート、ファイルの組み合わせ、
端末)単位で別途保存され、セッションが終わった後も最後の進捗率と学習時刻が残ります。
「この学習をもう一度」が実際に復元する単語キュー自体(`progress`テーブル)も、
画面に表示される要約情報(`learning_log`テーブル)も、採点・保存のたびにこの
端末のlocalStorageへ即座にコピーを残し、同時にSupabaseへの保存も試みます —
そのためネットワークが一時的に切れても、Supabase自体が未設定でも、同じ端末の
中では再開機能が常に動作します。

同じ番号を複数の端末(Mac・デスクトップ・iPhone・iPadなど)で使う場合、以前は後で
保存した端末が前の端末の記録を静かに上書きしていました。今は端末ごとに別々の記録を
残して全て保存しつつ、番号さえ同じなら端末を変えても正確に何番目の単語まで見たかが
そのまま引き継がれます: 学習・試験・文章パートは画面を開くとその組み合わせで
最後に保存された記録を(どの端末で保存したかに関わらず)自動で見つけて引き継ぎ、
練習パートは「続きから練習する」一覧から好きな記録を自分で選んで引き継ぎます。
同じファイルの組み合わせでも「続きから練習する」/「学習記録の管理」一覧には端末
ごとに分けて表示され、それぞれどの端末(設定 &gt; 「この端末の名前」で自分で
名付けられる)の記録か日付と一緒に表示されるので、不要な記録は自分で見て選んで
削除できます — 続きから始めるときは常に最新の記録を自動で使いますが、削除は
端末ごとに一つずつしかできないため、他の端末の記録が誤って一緒に消えることは
ありません。
...
- **発音を聞く** — 学習・練習画面の単語横のスピーカーアイコンを押すと、端末内蔵の
  ...
  キーボードの`S`でも聞けます(練習画面は正解を開く前だけ — 開いた後は`S`が採点
  ショートカット(分からない)としてそのまま使われます)。学習・練習画面ともボタンの
  横に`[S]`ショートカット表示があります。
...
- **今日の目標 + 進捗リング** — Duolingoの日次目標に相当します。設定で1日の目標数
  (5〜500語、よく使う30/50/100/150/200/300語のプリセット付き)を決めておくと、
  ホーム画面に今日何語採点したかを円形の進捗リングで表示し、今すぐ復習すべき
  単語数も併せて案内します(採点記録が一つもなければこのカードは表示されません)。
- **学習ヒートマップ / 習熟度分布 / 復習予報** — Ankiのレビューヒートマップ、カード
  状態分布、Future Dueグラフに相当します。「学習統計」画面で直近16週間の学習量を
  GitHub風の草グラフで、これまで採点した単語が完璧/少し分かる/曖昧/分からないの
  どこに集中しているかを棒グラフで、今後7日間に何日にいくつ復習が戻ってくるかを
  グラフで表示します。
- **セッションサマリー画面** — Duolingoのレッスン完了画面に相当します。以前は練習を
  終えると「キューの全ての練習を完了しました。」の一行だけでしたが、今回のセッションで
  完璧/少し分かる/曖昧/分からないをそれぞれ何回採点したか、合計採点回数・所要時間・
  完璧+少し分かるの割合まで見せるサマリーに変えました。
- **採点の取り消し** — AnkiのUndo(Ctrl+Z)に相当します。練習画面で直前に押した点数が
  誤クリックだったとき、「取り消し」ボタン(またはBackspaceキー)で一つ前の状態に
  戻せます。画面上のキュー・進捗率だけでなく、サーバーに保存されたその単語の
  習熟度(SRS間隔含む)も採点前の値に戻すため、誤って押した点数が長期の復習
  スケジュールに残りません。一段階のみ取り消し可能で、次に採点すると取り消し対象は
  その採点の直前に更新されます。
...
たいときに使います。練習パートはここで削除すると「続きから練習する」カードからも
一緒に消えます(両方消してこそ本当のリセットになるため)。逆に練習画面の「続きから
練習する」カードからも直接「削除」ボタンで消せるので、設定まで行かなくても不要な
進捗(0個のものなど)を整理できます。一覧の読み込みに失敗したときは「再試行」
ボタンが表示されます。
...
| カードの文字サイズ調整 (−/+/デフォルト) | デスクトップではブラウザ拡大なしだと文字が小さいという意見から、学習/練習/試験/文章の全パートに80〜180%の倍率調整を追加しました |
| 練習画面「⚙設定」ポップオーバー(ラウンドサイズ・採点ボタンサイズ・非表示・アニメーション) | デスクトップを大型テレビに接続して使う際、採点ボタンが画面に対して大きすぎるという意見から、普段は歯車アイコン一つだけが見え、押すと調整UIが展開されるようにしました |
| アニメーション(マスコット)のデフォルトOFF | リアクションイラストが重く感じるという声が多く、デフォルトをON→OFFに反転しました(いつでも再度ONにできます) |
| ヒント表示設定(区間ごとの色・サイズ・非表示) | 単語ヒントが読み方/漢字分解/総合/暗記のコツ/場面/例文/活用形/紛らわしい単語に分かれており、人によって必要な区間が違うため、設定で区間ごとに色・サイズを変えたり非表示にできるようにしました。漢字1/漢字2の見出し行だけ別途強調するオプションもあります |
...
src/
  ...
  components/                    # FileSelector, FlashCard(3Dフリップ), Mascot, SessionInfoPanel, UsageGuide, PageHeader, PageTransition, Confetti, FeedbackFlash, StreakFlame,
                                  # FontSizeControl, PracticeSettingsPopover, SessionSummary, HintThemeSettings, TodayPanel, StudyHeatmap, MasteryBreakdownBar, DueForecastChart など
  contexts/                      # ThemeContext(ダークモード), FocusModeContext(集中モード)
  lib/                           # parser, queue(忘却曲線), mastery(習熟度), insights(ヒートマップ・習熟度分布・復習予報), hintTheme(ヒント区間別テーマ),
                                  # learningLog(ダッシュボード), streak(連続学習日), favorites(お気に入り), matchScores(マッチングゲーム記録), github(サーバー専用), supabase
  hooks/                         # useUserId(マイ番号), useFontScale, useRoundSize, useScoreButtonPrefs, useDailyGoal, useHintTheme, useKeyboardShortcuts
...
`schema.sql`は`create table if not exists`/`add column if not exists`が中心のため
**既にSupabaseを設定済みのデプロイに対して再実行しても安全です** — 後から列が
追加されるたびに(例: 端末ごとの記録を区別する`device_id`)このファイル全体を
SQL Editorに再度貼り付けて実行すれば、新しい列だけが追加され既存データはそのまま
残ります。
...
| 조금 앎 | 60 | 다음 3~5라운드 뒤 구간 | 헷갈림보다 여유를 두고 한 번 더 |
| 헷갈림 | 40 | 다음 라운드 또는 3~5라운드 뒤(50%) | 조금 앎보다 이르게 다시 |
| 모름 | 0 | 이번 라운드 안(최소 3장 뒤) | 곧바로 다시 |

완벽함만 "이번엔 충분히 안 것"으로 보고 즉시 완료 처리해 진행률에 반영됩니다. 조금
앎·헷갈림·모름은 전부 이번 세션 큐에 다시 꽂혀 한 번 더 만나는데, 그중 조금 앎은
이미 더 잘 아는 단어이므로 헷갈림보다 여유를 둔(3~5라운드 뒤) 구간에서만 다시
등장합니다 — 값을 즉시 완료로 확정하는 게 아쉽다는 의견을 반영해, "그래도 한 번 더
확인은 하고 넘어가는" 쪽으로 바꿨습니다(예전에는 완벽함만 완료로 쳐서 진행률이 잘
안 올라간다는 정반대 의견도 있었는데, 지금은 완벽함만 즉시 완료로 확정하고 나머지
세 단계는 모두 재등장시켜 두 요구 사이의 균형을 잡았습니다).

모름(0)은 이번 라운드 안(최소 3장 뒤)에서 무작위 위치에, 헷갈림(40)은 다음
라운드 또는 그보다 몇 라운드 뒤(50% 확률로 둘 중 하나) 구간에서, 조금 앎(60)은
항상 그보다 나중인 3~5라운드 뒤 구간(헷갈림의 범위와 겹쳐도 무방)에서 무작위
위치에 다시 꽂습니다(`src/lib/queue.ts`의 `requeuePosition`/`requeueRangeBounds`
— 채점 버튼에 표시되는 "N~M번째" 숫자도 이 함수 하나에서 그대로 가져와, 라벨과
실제 동작이 항상 일치합니다). 라운드 크기는 아래 "마이크로 러닝"에서 사용자가 직접
조절할 수 있어, 재등장 간격도 그 값을 그대로 따라갑니다.

세 가지 설계 의도가 있습니다.

- 고정값이 아니라 구간에서 무작위로 뽑습니다. 위치를 고정하면 사용자가 순서를
  외워버려서, 단어가 아니라 "순서"를 기억하게 됩니다.
- 절대 개수가 아니라 라운드 크기 기준으로 계산합니다. 라운드를 크게 잡은
  사람일수록 틀린 단어가 돌아오는 간격도 자연스럽게 늘어납니다.
- 채점 직후 방금 본 단어가 바로 다음 카드로 다시 나오지 않도록 최소 3장의
  간격(`MIN_GAP`)을 항상 보장합니다.
...
### 마이크로 러닝 — 조절 가능한 라운드와 자주 틀리는 단어

한 번에 수백 개를 몰아서 풀게 하면 중간에 지치기 쉽습니다. 그래서 연습을 "라운드"
단위로 쪼갰습니다. 기본값은 15개지만, 하루 30개만 가볍게 하는 사람과 200개 넘게
몰아서 하는 사람의 리듬이 달라 연습 화면 우상단 "⚙ 설정" 팝오버에서 5~50개
사이로 직접 조절할 수 있습니다(기기별 저장, 학습 기록과는 무관). 라운드를 채울
때마다 짧은 완료 화면이 뜨고, 화면 상단 진행률 바(젤리 스타일)도 전체가 아니라
이번 라운드 기준으로 다시 채워집니다. 세 번 이상 헷갈림·모름으로 채점된 단어는
정답 화면에 "자주 틀리는 단어" 배지가 붙어서, 어떤 단어가 특히 안 외워지는지
바로 알 수 있습니다.

### 간격 반복(SRS) — 오늘의 복습

완벽함(100점)으로 채점한 단어는 7일 후, 조금 앎(60점)으로 채점한 단어는 3일 후에
다시 볼 수 있도록 고정 간격을 적용합니다(예전에는 SuperMemo-2처럼 맞힐 때마다
간격이 점점 늘어나는 방식이었으나, 채점 4단계와 궁합이 안 맞아 점수별 고정값으로
단순화했습니다). 복습일이 아직 안 된 단어는 그날 연습 스택에서 아예 제외되고,
간격이 다 된 단어는 "복습" 화면의 "오늘의 복습" 카드에 모여서, 지금 다시 봐야 할
단어만 골라 바로 연습을 시작할 수 있습니다.
...
- 드롭다운에서 조합을 고르고 **[이 학습 다시 하기]**를 누르면, 그때 썼던 파일 선택과
  모드(이름만/뜻만/랜덤)가 그대로 복원되면서 자동으로 단어 화면까지 진입 — 그 조합에
  저장된 진행이 있으면(다른 기기에서 저장한 것이라도) 처음부터 다시 섞지 않고 큐와
  완료 개수를 정확한 그 지점 그대로 이어갑니다.

파일 조합별 진행 기록은 (사용자 번호, 파트, 파일 조합, 기기) 단위로 별도 저장되어,
세션이 끝난 뒤에도 마지막 진행률과 학습 시각이 남습니다. "이어서 연습하기"가 실제로
다시 펼치는 단어 큐 자체(`progress` 테이블)도, 화면에 보이는 요약 정보
(`learning_log` 테이블)도 둘 다 채점/저장 시점에 이 기기의 localStorage에 즉시
사본을 남기고 Supabase에도 동시에 저장을 시도합니다 — 그래서 네트워크가 일시적으로
끊기거나 Supabase 설정 자체가 안 되어 있어도 같은 기기 안에서는 이어서 연습하기가
항상 동작합니다.

같은 번호를 여러 기기(맥북·데스크탑·아이폰·아이패드 등)에서 쓸 때, 예전에는 나중에
저장한 기기가 이전 기기의 기록을 조용히 덮어썼습니다. 이제는 기기마다 별도 기록을
남겨 전부 보존하면서도, 번호만 같으면 기기를 바꿔도 정확히 몇 번째 단어까지
봤는지 그대로 이어집니다: 학습/시험/지문 파트는 화면을 열면 이 조합에서 가장
최근에 저장된 기록을(어느 기기에서 저장했든) 자동으로 찾아 이어받고, 연습 파트는
"이어서 연습하기" 목록에서 원하는 기록을 직접 골라 이어받습니다. 같은 파일
조합이라도 "이어서 연습하기"/"학습 기록 관리" 목록에는 기기별로 따로 표시되고,
각 기록에 어느 기기(설정 &gt; "이 기기 이름"에서 직접 이름을 지어둘 수 있음)의
것인지 날짜와 함께 보여줘서, 필요 없는 기록은 직접 보고 골라 지울 수 있습니다 —
이어서 볼 때는 가장 최신 기록을 자동으로 쓰지만, 지울 때는 기기별로 하나씩만
지울 수 있어 다른 기기의 기록이 실수로 같이 지워지지 않습니다.
...
- **발음 듣기** — 학습·연습 화면 단어 옆 스피커 아이콘을 누르면, 기기에 내장된 음성
  ...
  동작합니다. 키보드 `S`로도 들을 수 있고(연습 화면은 정답을 열기 전까지만 — 연 뒤엔
  `S`가 채점 단축키(모름)로 그대로 쓰입니다), 학습·연습 화면 모두 버튼 옆에 `[S]`
  단축키 표시가 붙어 있습니다.
- **메모장(memo)** — 학습·연습 화면 오른쪽 아래 "memo" 버튼으로 손글씨 메모장을 열
  ...
  있습니다.
- **오늘의 목표 + 진행 링** — Duolingo의 일일 목표에 해당합니다. 설정에서 하루 목표
  개수(5~500개, 자주 쓰는 30/50/100/150/200/300개 프리셋 포함)를 정해두면, 홈
  화면에 오늘 몇 개를 채점했는지 원형 진행 링으로 보여주고, 지금 복습할 때가 된
  단어 수도 함께 안내합니다(채점 기록이 하나도 없으면 이 카드는 뜨지 않습니다).
- **학습 히트맵 / 숙련도 분포 / 복습 예보** — Anki의 리뷰 히트맵, 카드 상태 분포,
  Future Due 그래프에 해당합니다. "학습 통계" 화면에서 최근 16주 학습량을 깃허브
  잔디 스타일로, 지금까지 채점한 단어가 완벽/조금 앎/헷갈림/모름 중 어디에 몰려
  있는지 막대로, 앞으로 7일간 며칠에 몇 개씩 복습이 돌아오는지 그래프로 보여줍니다.
- **세션 요약 화면** — Duolingo의 레슨 완료 화면에 해당합니다. 예전에는 연습을 다
  끝내면 "대기열의 모든 연습을 완료했습니다." 한 줄뿐이었는데, 이번 세션에서
  완벽함/조금 앎/헷갈림/모름을 각각 몇 번 채점했는지, 총 채점 횟수·소요 시간·
  완벽+조금 앎 비율까지 보여주는 요약으로 바꿨습니다.
- **채점 되돌리기** — Anki의 Undo(Ctrl+Z)에 해당합니다. 연습 화면에서 방금 누른
  점수가 misclick이었을 때 "되돌리기" 버튼(또는 Backspace 키)으로 바로 전 상태로
  되돌릴 수 있습니다. 화면에 보이는 큐·진행률뿐 아니라 서버에 저장된 그 단어의
  숙련도(SRS 간격 포함)까지 채점 전 값으로 되돌리므로, 잘못 누른 점수가 장기
  복습 일정에 남지 않습니다. 한 단계만 되돌릴 수 있고, 다음 채점을 하면 되돌리기
  대상이 그 채점 직전으로 다시 바뀝니다.
...
재고 싶을 때 사용합니다. 연습 파트는 여기서 삭제하면 "이어서 연습하기" 카드에서도
함께 사라집니다(둘 다 지워야 진짜로 리셋된 것이므로). 반대로 연습 화면의 "이어서
연습하기" 카드에서도 바로 "삭제" 버튼으로 지울 수 있어, 설정까지 안 가도 필요 없는
진행(예: 0개짜리)을 정리할 수 있습니다. 목록을 못 불러왔을 때는 "다시 시도" 버튼이
뜹니다.
...
| 카드 글자 크기 조절 (−/+/기본값) | 데스크탑에서 브라우저 확대 없이는 글씨가 작다는 의견에 따라, 학습/연습/시험/지문 전 파트에 80~180% 배율 조절을 넣었습니다 |
| 연습 화면 "⚙ 설정" 팝오버 (라운드 크기·채점 버튼 크기·숨기기·애니메이션) | 데스크탑을 대형 TV에 연결해 쓸 때 채점 버튼이 화면에 비해 과하게 크다는 의견으로, 평소엔 톱니바퀴 하나만 보이다가 눌러야 조절 UI가 펼쳐지게 했습니다 |
| 애니메이션(마스코트) 기본값 끄기 | 반응 일러스트가 무겁게 느껴진다는 의견이 많아, 기본값을 켜짐 → 꺼짐으로 뒤집었습니다(원하면 다시 켤 수 있음) |
| 힌트 표시 설정 (구간별 색·크기·숨김) | 단어 힌트가 읽기/한자 분해/종합/치트키/장면/예문/활용/헷갈리는 단어로 나뉘어 있는데, 사람마다 필요한 구간이 달라 설정에서 구간별로 색·크기를 바꾸거나 아예 숨길 수 있게 했습니다. 한자1/한자2 제목 줄만 따로 강조하는 옵션도 있습니다 |
...
src/
  ...
  components/                    # FileSelector, FlashCard(3D 플립), Mascot, SessionInfoPanel, UsageGuide, PageHeader, PageTransition, Confetti, FeedbackFlash, StreakFlame,
                                  # FontSizeControl, PracticeSettingsPopover, SessionSummary, HintThemeSettings, TodayPanel, StudyHeatmap, MasteryBreakdownBar, DueForecastChart 등
  contexts/                      # ThemeContext(다크모드), FocusModeContext(집중 모드)
  lib/                           # parser, queue(망각곡선), mastery(숙련도), insights(히트맵·숙련도 분포·복습 예보), hintTheme(힌트 구간별 테마),
                                  # learningLog(대시보드), streak(연속 학습일), favorites(즐겨찾기), matchScores(매칭 게임 기록), github(서버 전용), supabase
  hooks/                         # useUserId(내 번호), useFontScale, useRoundSize, useScoreButtonPrefs, useDailyGoal, useHintTheme, useKeyboardShortcuts
...
`schema.sql`은 `create table if not exists`/`add column if not exists` 위주라
**이미 Supabase를 설정해둔 배포에도 그대로 다시 실행해도 안전합니다** — 나중에
컬럼이 추가될 때마다(예: 기기별 기록 구분을 위한 `device_id`) 이 파일 전체를
SQL Editor에 다시 붙여넣어 실행하면 새 컬럼만 추가되고 기존 데이터는 그대로
남습니다.
