...
中では再開機能が常に動作し、接続が復旧すればサーバーと自動的にマージされます
(他の端末間の同期のみSupabaseが必要)。両方の値が異なる場合は`updated_at`が
より新しい方を使います。
...
끝난 뒤에도 마지막 진행률과 학습 시각이 남습니다. "이어서 연습하기"가 실제로 다시
펼치는 단어 큐 자체(`progress` 테이블)도, 화면에 보이는 요약 정보(`learning_log`
테이블)도 둘 다 채점/저장 시점에 이 기기의 localStorage에 즉시 사본을 남기고
Supabase에도 동시에 저장을 시도합니다 — 그래서 네트워크가 일시적으로 끊기거나
Supabase 설정 자체가 안 되어 있어도 같은 기기 안에서는 이어서 연습하기가 항상
동작하고, 연결이 복구되면 서버와 자동으로 병합됩니다(다른 기기 간 동기화만
Supabase가 필요). 두 저장소 값이 다르면 `updated_at`이 더 최신인 쪽을 씁니다.
