  ...
  },
  {
    ...
    body: (
      <>
        ...
        채점합니다. <b>완벽함과 조금 앎은 큐(대기열)에서 빠지고 완료율에 반영</b>되며,
        헷갈림·모름은 큐의 앞쪽 구간에 다시 꽂혀서 금방 다시 만납니다. 위치는 매번 구간
        안에서 무작위로 정해져서, 단어가 아니라 &ldquo;순서&rdquo;를 외워버리는 것을
        막습니다. <b>15개 단위 라운드</b>로 나눠서, 15개를 채울 때마다 작은 완료 화면이
        뜨고 진행률 바도 그 라운드 기준으로 다시 채워집니다 — 한 번에 수백 개를 몰아보는
        대신 짧게 짧게 성취감을 느끼도록 했습니다. 세 번 이상 틀린 단어는 정답 화면에
        &ldquo;자주 틀리는 단어&rdquo; 배지가 붙습니다. 채점 결과(단어별 숙련도)는
        서버에 저장되어, 다음에 같은 번호로 연습을 다시 시작해도 예전에 약했던 단어가
        먼저 나옵니다 — 세션이 끝나도 기억됩니다.
      </>
    ),
  },
  {
  ...
  },
  {
    ...
    body: (
      <>
        ...
        기록보다 빠르면 새 기록으로 저장됩니다. 하단 탭의 <b>매칭게임</b>에서 바로
        시작할 수 있습니다.
      </>
    ),
  },
...
const FEATURE_ITEMS: GuideItem[] = [
  {
    ...
    body: (
      <>
        ...
        합니다. 방금 하다가 나온 조합과 정확히 같으면 처음부터 다시 섞지 않고{" "}
        <b>큐와 완료 개수를 그대로 이어서</b> 보여줍니다.
      </>
    ),
  },
  {
  ...
  {
    title: "복습 — 완벽함/조금 앎 단어 다시 보기",
    body: (
      <>
        연습에서 <b>완벽함(100)</b> 또는 <b>조금 앎(60)</b>으로 채점한 단어는 &ldquo;설정
        &gt; 복습&rdquo; 또는 &ldquo;더보기 &gt; 복습&rdquo;에서 모아볼 수 있습니다.
        목록에서 바로 그 단어들로만 연습을 시작할 수 있고, 다시 헷갈리는 단어는
        <b>초기화</b>해서 다음 연습에서 아직 안 본 단어처럼 우선 출제되게 할 수 있습니다.
      </>
    ),
  },
  {
    title: "간격 반복(SRS) — 오늘의 복습",
    body: (
      <>
        조금 앎(60점)으로 채점한 단어는 <b>3일 후</b>, 완벽함(100점)으로 채점한 단어는{" "}
        <b>7일 후</b>에 다시 스택에 올라옵니다. 그 전까지는 오늘 연습 스택에서 아예
        빠져 있다가, 복습일이 되면 다시 나타납니다. 그 주기가 다 된 단어는
        &ldquo;복습&rdquo; 화면 위쪽에 <b>&ldquo;오늘의 복습&rdquo;</b> 카드로 모여서,
        지금 다시 봐야 할 단어만 골라 바로 연습을 시작할 수 있습니다.
      </>
    ),
  },
  {
    ...
    body: (
      <>
        학습·연습 화면의 단어 옆 스피커 아이콘을 누르면, 기기에 내장된 음성 합성으로
        그 단어의 발음을 들을 수 있습니다. 별도 서버나 API 없이 브라우저 기능만으로
        동작합니다.
      </>
    ),
  },
  {
    ...
    body: (
      <>
        학습·연습 화면 오른쪽 아래의 <b>memo</b> 버튼을 누르면 필기용 메모장이 뜹니다.
        기본적으로 손가락 터치로도 그려지고, <b>펜슬 모드</b>를 켜면 애플펜슬(또는
        마우스)만 그려지고 손가락은 무시됩니다(아이패드에서 손바닥을 대고 써도 안
        그려지는 팔레트 거부). <b>-/+</b>로 펜 굵기를 바꿀 수 있고, 제목 표시줄(점
        6개 아이콘)을 끌면 메모장 위치를 옮길 수 있으며 오른쪽 아래 모서리를 끌면
        크기를 바꿀 수 있습니다. <b>지우기</b>로 전체를 지울 수 있고, <b>닫기</b>를
        눌러도 그리던 내용은 남아있어서 memo 버튼을 다시 누르면 이어서 쓸 수 있습니다.
      </>
    ),
  },
  {
