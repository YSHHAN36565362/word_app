...
export default function FavoritesPage() {
  ...
  useEffect(() => {
    ...
    loadFavorites(userId).then((w) => {
      setWords(w);
      setLoading(false);
    });
  }, [ready, userId]);

  async function unstar(idx: number) {
    const w = words[idx];
    setWords((prev) => prev.filter((_, i) => i !== idx));
    await removeFavorite(userId, w);
  }
  ...
  if (ready && !userId) {
    return (
      <div className="mx-auto max-w-xl px-4 pt-6 pb-8">
        <PageHeader icon="즐" accent="var(--amber)" iconColor="#2b2200" title="즐겨찾기" />
  ...
  return (
    <div className="mx-auto max-w-xl px-4 pt-6 pb-8">
      <PageHeader
        ...
        iconColor="#2b2200"
        title="즐겨찾기"
        subtitle="학습·연습 화면에서 별표(☆)를 누른 단어가 여기 모입니다."
      />
      ...
      {loading ? (
        ...
        <>
          <div className="mt-4 flex items-center justify-between">
            ...
            <Link href="/practice?from=favorites" className="btn-3d btn-blue px-3 py-1.5 text-xs">
              즐겨찾기로 연습
            </Link>
          </div>
            ...
            {words.map((w, idx) => (
              <div key={`${wordKey(w)}-${idx}`} className="study-card p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button onClick={() => unstar(idx)} className="text-lg shrink-0" aria-label="즐겨찾기 해제">
                    ★
                  </button>
                </div>
                {w.hint.trim() && (
